...
public class Summariser implements Operator{
    @Override
    public void execute(Stack<Float> stack, Definition defParams, Reader reader){
        ...
        try {
            ...
            throw new OperatorException("Can not summarise. Stack is empty.");
        }
        try {
            b = stack.pop();
        }
            ...
            stack.push(a);
            throw new OperatorException("Can not summarise. There is only 1 element in stack.");
        }
        stack.push(a + b);
    }
}
